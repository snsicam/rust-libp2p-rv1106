...
└──────────────────────────┬───────────────────────────────────────┘
              ...
              ▼                          ▼
┌─────────────────────────┐  ┌──────────────────────────┐
│ 模块 2: P2P DeviceCam    │  │ 模块 3: Mobile APP       │
...
```
阶段 1: 入网 + 信令
  DeviceCam ──TCP──▶ Relay Server ◀──QUIC── Mobile APP
  1. DeviceCam 在 Relay 预约 (Reserve)
  2. DeviceCam PeerId 存入 Redis/HTTP
  3. Mobile 通过 HTTP API 查询 DeviceCam PeerId
...
```rust
// 端口规划
TCP 4001 : libp2p 原生节点 (DeviceCam 用 TCP 连, Mobile 用 QUIC 连)
...
2. HTTP API — 简单信令服务:
   - `POST /register` — DeviceCam 注册 PeerId + 端口
   - `GET /peers` — Mobile 查询在线 DeviceCam 列表
3. Identify — 节点身份交换
...
协议定义详见上面 **模块2 DeviceCam** 中的 `MediaPacket` 实现。核心设计：
...
│
├── [Day2] HTTP 信令 API
│   ├── POST /register  (DeviceCam 注册 PeerId)
│   ├── GET /peers       (查询在线列表)
│   └── 简单内存存储
│
├── [Day3] 部署到云服务器
...
### Phase 2: DeviceCam (RV1106) (7-10天)
...
| 资源 | 需求 | 是否满足 |
|------|------|----------|
| CPU (Cortex-A7) | SDK 回调 + libp2p node | ✅ H.265 硬件编码, DeviceCam 只转发不解码 |
...
这样 DeviceCam 和 Mobile 只需要知道 Relay Server 的 IP 和 PeerId, 其余通过 HTTP API 自动发现。
