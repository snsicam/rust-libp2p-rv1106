...
| 19 | 2026-07-16 | NAT诊断 | DeviceCam Relay重连后被误判为Symmetric NAT，实际是Full Cone NAT | Relay重连后QUIC本地端口变化（34696→47585），`diagnose()`将不同连接的观测端口混合比较，端口不一致被误判为Symmetric NAT | `record_observed()`新增端口一致性检查：新观测端口与历史不一致时清除旧观测（Relay重连场景），只保留当前连接的观测数据 | `device-cam/src/net_diag.rs`, `mobile-core/src/net_diag.rs` |
