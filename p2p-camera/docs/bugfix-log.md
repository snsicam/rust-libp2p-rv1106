...
| 7 | 2026-06-25 | Viewer (CLI) | DCUtR 直连升级后立即报 "Gateway connection closed" 触发重连，但直连实际还在 | DCUtR 打洞成功后 libp2p 自动关闭冗余的 circuit relay 连接，触发 `ConnectionClosed`；旧代码不看剩余连接数直接判定断开 | `ConnectionClosed` 分支检查 `num_established` 字段：`==0` 才真正断开触发重连；`>0` 说明只是 circuit 关闭、直连仍在，继续运行 | `mobile-core/examples/viewer_cli.rs` |
