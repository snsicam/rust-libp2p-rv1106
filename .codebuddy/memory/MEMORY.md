...
- **关键帧判定只在 viewer 侧字节扫描, 结果写入 MediaPacket.flags 的 FLAG_VIDEO_KEYFRAME bit (0x04), cam 不计算/不传 keyframe**: cam 侧 C 回调 `rk_camera.c frame_callback_t` 已**移除** `is_keyframe` 形参, Rust `rk_video_source.rs on_frame` 签名 `fn(chn_id, data, len, pts_us)`, 均 `MediaPacket::video(ts, data)` (无 is_keyframe 形参)。关键帧完全由 viewer 字节扫描 `is_nal_keyframe` (H.265 IRAP 16-21 / H.264 IDR 5) 判定 (receiver 自包含、不信任对端 flag)。**2026-07-21 优化去重**: 关键帧不再用独立 `bool` 字段, 而是复用 `MediaPacket.flags` 的 bit 2 (`FLAG_VIDEO_KEYFRAME=0x04`), 与音频 codec 低 2 位(0=PCM/1=AAC/2=G711A/3=G711U) 无冲突; viewer 的 `receive_frames` 每帧扫描一次 `is_nal_keyframe`, 命中则 `packet.flags |= FLAG_VIDEO_KEYFRAME`, 并提供 `is_keyframe()` 方法封装读取; 下游 `drain_to_latest_keyframe` / `media_viewer.rs` 直接调 `packet.is_keyframe()` 复用, 消除原先 media_viewer 每帧 2 次 + drain 每批 1 次的重复字节扫描 (总扫描从每帧 2+ 次降到每帧 1 次, 集中到 receiver)。cam 端 `rk_video_source.rs` 与 `try_decode` 构造时 flags 不含该 bit。**JNI 桥 `nativePollVideoFrame/AudioFrame` 仍只转发 `[PTS 8B]+[raw data]`, 但现可顺带用可靠的 `packet.is_keyframe()` 转发给原生 APP** (修复原生 APP 无法区分关键帧的潜在问题, 待实施)。注意: `MediaPacket` 的音频 `flags` 字段目前也**未**经 JNI 转发给 Kotlin/Swift, 属潜在 bug (原生 APP 无法区分 PCM/G711/AAC)。
